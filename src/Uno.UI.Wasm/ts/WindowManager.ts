namespace Uno.UI {
	...
	export class WindowManager {
		...
		private static dispatchPointerEventMethod: any;
		...
		*/
		...
		private static pointerEnterEventHandler = (event: PointerEvent) => {
			const e = event as any;
			const element = event.currentTarget as HTMLElement | SVGElement;
		...
		private static pointerLeaveEventHandler = (event: PointerEvent) => {
			const e = event as any;
			const element = event.currentTarget as HTMLElement | SVGElement;
		...
		private static pointerMoveEventHandler = (event: PointerEvent) => {
			const element = (event.currentTarget as HTMLElement | SVGElement);
		...
		private static pointerEventHandler = (event: PointerEvent) => {
		...
		private registerEventOnViewInternal(
			...
			if (eventName == "pointerenter") {
				const enterPointerHandler = (event: PointerEvent) => {
					const e = event as any;
				...
				const leavePointerHandler = (event: PointerEvent) => {
					const e = event as any;
				...
				const movePointerHandler = (event: PointerEvent) => {
		...
		private static initMethods() {
			if (WindowManager.isHosted) {
				...
				if (!WindowManager.dispatchPointerEventMethod) {
					WindowManager.dispatchPointerEventMethod = (<any>Module).mono_bind_static_method("[Uno.UI] Windows.UI.Xaml.UIElement:DispatchPointerEvent");
				}
			}
		}
